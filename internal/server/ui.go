...
const dashHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Booking</title>
...
<div class="hdr"><h1><span>&#9670;</span> BOOKING</h1><button class="btn btn-p" id="add-btn">+ Add</button></div>
<div class="main"><div class="tabs" id="tabs"><div class="tab active" onclick="switchTab('services')">Services</div><div class="tab" onclick="switchTab('appointments')">Appointments</div><div class="tab" onclick="switchTab('availability')">Availability</div></div><div class="tab-content active" id="tab-services">
<div class="stats" id="stats-services"></div>
<div class="toolbar">
<input class="search" id="search-services" placeholder="Search..." oninput="renderRes('services')"></div>
<div class="count-label" id="count-services"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Service Name</th><th>Duration (min)</th><th>Price ($)</th><th>Description</th><th>Active</th><th></th></tr></thead><tbody id="tbody-services"></tbody></table></div></div><div class="tab-content" id="tab-appointments">
<div class="stats" id="stats-appointments"></div>
<div class="toolbar">
<input class="search" id="search-appointments" placeholder="Search..." oninput="renderRes('appointments')"><select class="filter-sel" id="filter-appointments-status" onchange="renderRes('appointments')"><option value="">All Status</option><option value="Pending">Pending</option><option value="Confirmed">Confirmed</option><option value="Completed">Completed</option><option value="Cancelled">Cancelled</option></select></div>
<div class="count-label" id="count-appointments"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Client Name</th><th>Email</th><th>Phone</th><th>Service</th><th>Date</th><th>Time</th><th>Status</th><th>Notes</th><th></th></tr></thead><tbody id="tbody-appointments"></tbody></table></div></div><div class="tab-content" id="tab-availability">
<div class="stats" id="stats-availability"></div>
<div class="toolbar">
<input class="search" id="search-availability" placeholder="Search..." oninput="renderRes('availability')"><select class="filter-sel" id="filter-availability-day_of_week" onchange="renderRes('availability')"><option value="">All Day</option><option value="Monday">Monday</option><option value="Tuesday">Tuesday</option><option value="Wednesday">Wednesday</option><option value="Thursday">Thursday</option><option value="Friday">Friday</option><option value="Saturday">Saturday</option><option value="Sunday">Sunday</option></select></div>
<div class="count-label" id="count-availability"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Day</th><th>Start Time</th><th>End Time</th><th>Active</th><th></th></tr></thead><tbody id="tbody-availability"></tbody></table></div></div></div><div class="modal-bg" id="mbg" onclick="if(event.target===this)closeModal()"><div class="modal" id="mdl"></div></div><script>var A="/api",data={},editId=null,activeRes="services";var resCfg={"services":{d:"Services",f:[{n:"service_name",l:"Service Name",t:"text",r:true,o:[]},{n:"duration_minutes",l:"Duration (min)",t:"integer",r:false,o:[]},{n:"price",l:"Price ($)",t:"number",r:false,o:[]},{n:"description",l:"Description",t:"textarea",r:false,o:[]},{n:"active",l:"Active",t:"checkbox",r:false,o:[]}]},"appointments":{d:"Appointments",f:[{n:"client_name",l:"Client Name",t:"text",r:true,o:[]},{n:"client_email",l:"Email",t:"email",r:false,o:[]},{n:"client_phone",l:"Phone",t:"phone",r:false,o:[]},{n:"service",l:"Service",t:"text",r:false,o:[]},{n:"date",l:"Date",t:"date",r:true,o:[]},{n:"time",l:"Time",t:"text",r:true,o:[]},{n:"status",l:"Status",t:"select",r:false,o:["Pending","Confirmed","Completed","Cancelled"]},{n:"notes",l:"Notes",t:"textarea",r:false,o:[]}]},"availability":{d:"Availability",f:[{n:"day_of_week",l:"Day",t:"select",r:true,o:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]},{n:"start_time",l:"Start Time",t:"text",r:true,o:[]},{n:"end_time",l:"End Time",t:"text",r:true,o:[]},{n:"active",l:"Active",t:"checkbox",r:false,o:[]}]},};

async function loadAll(){
for(var k in resCfg){
var r=await fetch(A+'/'+k).then(function(x){return x.json()});
var items=r[k]||[];
// Fetch extras for this resource and merge into each item
try{
...
// Check required (native fields only — custom fields are always optional)
...
// Add custom fields to the first resource tab
...
['services','appointments','availability'].forEach(function(r){renderRes(r)});
}
}else{
