...
// Tests for the additive trial-gate middleware on booking. Booking
...
//   7. End-to-end: tier=="none" → POST /api/services returns 402
//   8. End-to-end: tier=="paid" → POST /api/services returns 200
//   9. End-to-end: tier=="none" → GET /api/services still works (read-through)
...
// booking's existing limits.go doesn't expose a hook for that.

func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "booking-trial-test-*")
	if err != nil {
...
func newTestServer(t *testing.T, tier string) *Server {
	t.Helper()
	dir, err := os.MkdirTemp("", "booking-trial-srv-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
...
func TestShouldBlockWrite_NoneTier(t *testing.T) {
	...
	for _, c := range cases {
		req := httptest.NewRequest(c.method, "/api/services", nil)
...
func TestShouldBlockWrite_PaidTier(t *testing.T) {
	...
	for _, m := range []string{"POST", "PUT", "DELETE", "PATCH", "GET"} {
		req := httptest.NewRequest(m, "/api/services", nil)
		if s.shouldBlockWrite(req) {
...
func TestEndToEnd_NoneTierBlocksWrite(t *testing.T) {
	s := newTestServer(t, "none")
	body := bytes.NewBufferString(`{"service_name":"Massage","duration_minutes":60}`)
...
func TestPersistLicense_WritesFile(t *testing.T) {
	dir, err := os.MkdirTemp("", "booking-persist-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
...
func TestLoadLicenseFromDisk_RoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "booking-load-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
...
func TestLoadLicenseFromDisk_MissingReturnsEmpty(t *testing.T) {
	dir, err := os.MkdirTemp("", "booking-missing-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
